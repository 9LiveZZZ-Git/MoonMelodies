...
- **A local backend** (`PlanetProfile/API/server/`, FastAPI + uvicorn). A loopback-only HTTP/JSON + SSE server that fronts the engine through a warm pool of `ppworker` subprocesses — submit a model as JSON, watch progress over Server-Sent Events, and pull back results + artifacts. Loopback-bound with a per-session token, CORS/PNA handling for a GitHub-Pages frontend, and one job per worker (the engine is non-reentrant). Beyond single models it also serves **2-D grid sweeps** — both ExploreOgram (interior structure) and InductOgram (σ×D induction response) — and **amortized Bayesian inference** (`/infer`, sampling the downloaded SBI posteriors). See [Running the local backend](#running-the-local-backend).
- **A browser frontend** (`frontend/index.html`). A single self-contained page — no build step, no external assets — with a **one-prompt "Generate report"** flow: choose a body and its two-of-three hydrosphere constraints, and a single click produces the whole report, each section filling in as its job completes. The report is a to-scale radial interior "plate" plus a **publication-styled figure suite** (structure, thermodynamics, seismic, electrical, **tidal Love numbers**, **magnetic-induction response**) with real ticked, unit-labelled axes and physical-radius abscissae; **2-D sweep heatmaps** with labelled contours (ExploreOgram interior structure + InductOgram σ×D induction); and a **posterior corner plot** (1σ/2σ contours, median ±68%) for Bayesian inference across five bodies. Every figure and dataset is **exportable** — per section, or as one report bundle (3× PNGs + CSVs + raw JSON + a provenance manifest, via a self-contained zip writer). It talks to the loopback backend with a session token; the backend can serve it same-origin via `--static-dir frontend`.
...
- **Bayesian interior inference** (`PlanetProfile.Inference`). MCMC (pocoMC) and simulation-based inference (`sbi`/`torch`) constrain interior parameters against tidal Love numbers, gravity, and magnetic-induction observables, with a full suite of posterior and diagnostic figures. The `/infer` service serves **amortized SBI posteriors for five bodies** — Europa and Titan (the upstream's validated 1M-simulation flows) plus **Enceladus, Ganymede, and Callisto trained locally** with the in-repo pipeline (structure cache → cached-forward training set → NSF flow → SBC gate; see [`PlanetProfile/Inference/TRAINING_MOONS.md`](PlanetProfile/Inference/TRAINING_MOONS.md)). Optional — `pip install -e ".[inference]"`.
...
The backend (`PlanetProfile/API/server/`) and the browser frontend (`frontend/index.html`) are both built; point the server at the page with `--static-dir frontend` and open the printed loopback URL. Both 2-D sweep modes are wired end-to-end — the ExploreOgram interior sweep and the InductOgram σ×D induction-response sweep. **In progress (see the spec):** packaging the page for static GitHub-Pages hosting against a user's local backend.
...
| `POST /runs` | submit a model as JSON → `202 {id}` — `mode:"single"`, `"exploreogram"` (interior sweep), or `"inductogram"` (σ×D induction sweep); `422` with field-level errors |
