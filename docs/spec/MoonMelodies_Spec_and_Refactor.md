...
- **Phase 2 (Bug-Fix Sweep)** — all confirmed defects in §7 closed (reload filename/glob errors, worker-count doubling, EOS-cache spawn-boundary miss, De Morgan inversion save-guard, CLI list-vs-scalar comparison, mutation-by-aliasing, and more), each with a regression test in `tests/test_phase2_regressions.py` (12 passing). The deferred `PRELOAD_EOS` item is fixed too: `GridPlanetProfileFunc` now shares picklable preloaded EOS with the `spawn` workers via a `Pool(initializer=…)` (per-EOS filtering keeps the expensive Perple_X tables, skips unpicklable SeaFreeze/Reaktoro handles); a parallel + `PRELOAD_EOS` grid reproduces the serial result bit-for-bit.
- **Phase 3 (API Boundary)** — the engine now has a declarative JSON contract in **`PlanetProfile/API/`**, the drop-in replacement for the CLI's `importlib`-a-`PP<Body>.py` step: `mapper.build_planet` constructs a `PlanetStruct` through a **whitelist derived from the struct schema itself** (only attributes that already exist on a fresh substruct and are JSON-compatible; EOS/interpolator holders are blocked; unknown attributes are rejected — no user file is ever imported). `ppworker.py` is the long-lived JSONL stdin/stdout harness (import once, snapshot a pristine `deepcopy(Params)`, then one job per line in an isolated `jobdir`, with the real stdout kept as a pristine JSON channel and all engine chatter sent to stderr). `results.py` serializes the single-run result (§3.2) with strict-JSON safety (complex → `{re,im}`, NaN/Inf → `null`) and a jobdir artifact manifest; `validate.py` enforces the up-front rules (two-of-three, `NO_H2O`→`qSurf`, body/comp enums, EOS-table allowlist + path-traversal defense); `schema.py` emits the `/schema` payload (input JSON Schema, output dictionary, enums) from the same whitelist. **Validated:** bit-for-bit parity against a CLI run (every summary scalar **and** the full per-layer `P_MPa` array identical on `PPTest1`), the worker driven as a real subprocess over JSONL, and a Europa run with **all six artifacts inside the jobdir and zero files written outside it**; 12 unit tests in `tests/test_api_phase3.py`. Two notes for the phases below: the ice-shell-thickness **input** field is the real engine attribute `zb_approximate_km` (the doc's illustrative `zb_km` is a *derived output*), and the request contract includes a top-level `planet` section for the GetPfreeze/GetTfreeze search bounds — omitting them silently shifts an iterative solve at the ~1e-13 level (the parity check caught exactly this). Caveat: `Test*` bodies write to an absolute install path by design of the test harness, so only real bodies are jobdir-isolated — which is what the backend runs.
- **Reproducibility — verified across fresh processes.** The old code had a reproducibility problem (per-run `Params` aliasing let flags bleed between successive runs in one process); Phase 0 fixed it (each run starts from a `deepcopy(configParams)`). To confirm the current code is deterministic, each model was run **in a brand-new OS process every time** (fresh interpreter, fresh imports, no shared in-process `EOSlist`/global state — the case where such bugs hide), fingerprinting the **entire** result with a SHA-256 over all summary scalars **plus all 23 per-layer arrays × every layer, the trade arrays, and the gravity/Love numbers**:

  | Model | Runs | Path | Result |
  |---|---|---|---|
  | `PPTest1` (core pipeline) | 5× fresh process | engine (`importlib`) | all identical — `412d7da3…` |
  | Europa (**+ gravity + magnetic induction**) | 3× fresh process | engine (`importlib`) | all identical — `6ba6e42c…` |
  | `PPTest1` (the new API path) | 3× fresh process | JSON → `build_planet` | all identical — `412d7da3…`, **byte-identical to the engine** |
...
**Bayesian inference — verification, models, and open options (2026-07-24).** A self-certification pass confirmed the inference *infrastructure* is intact in the fork: the Europa v3 2D structure cache survives the history purge, the forward emulator (cache load → k2/induction → likelihood) runs, and a real MCMC (`result_v3.pkl`, ESS≈4212) is available. A first-pass MAF flow trained locally on ~10k support-valid sims does **not** pass the ratified `validate_sbi` gates (crosscheck: 8/8 mean + 8/8 sigma + 6/8 median but **0/8 shape**; sbc: **3/8 calibrated** — α, Tb, salinity pass; the rheology/core params fail) — i.e. location/scale correct, fine shape/calibration not, exactly as expected for an under-budget flow. The gates are strict and behave ("do not tune"). **Rather than reproduce the science, the upstream's validated models were downloaded**: the HF Space ships **14 self-describing, GUI-deployed 1M-sim NSF artifacts** (~6.4 MB total) in `PlanetProfile/Inference/sbi_artifacts/` (Git-LFS), with `INDEX.md` documenting per-artifact gate status (e.g. `europa_galileo_v1p1_8D_posterior_1m.pt` = *ALL GATES PASS*; clipper v4/v5/v6 deployed). These were pulled and **housed locally**; they load and sample correctly in the fork (a v3 crosscheck ran end-to-end and scored 4/8 overall vs the local 10k flow's 0/8). Two caveats: (i) `crosscheck` is only meaningful against each artifact's **matched** reference MCMC (the ones the Space validated against, under `validation_reports/`), not our ad-hoc `result_v3.pkl`; (ii) the cache-**rebuild** path fails on a missing `Bulk.meanMotion_radps` field — the fork can *load and use* caches/models but cannot *regenerate* them without that fix.

**Open options for the Bayesian thread** (deferred; not on the Phase 4 critical path):
- **(a) Full-budget retrain** (100k+ sims + NSF, ~15–20 min) to reproduce a gate-passing flow locally — largely *obviated* by the download.
- **(b) Fix the `meanMotion_radps` rebuild gap** so structure caches / artifacts can be regenerated in-fork rather than only consumed.
- **(c) Ship the downloaded upstream artifacts + the MCMC path** — *recommended*: house the 14 artifacts + their matched reference MCMCs + gate reports via the install download flow (same pattern as the Perple_X tables and structure caches), re-certify each against its own reference, and expose them read-only in the UI. Treat originating *new* science (new bodies/priors) as a separate research effort.

Status: **inference infrastructure verified; upstream validated models downloaded and housed; MCMC path ship-ready; local SBI training needs a budget pass (or just use the downloaded flows).**
